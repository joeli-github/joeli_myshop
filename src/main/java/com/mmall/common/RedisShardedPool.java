...
public class RedisShardedPool {
	...
	private static ShardedJedisPool pool;//sharded jedis的连接池
	...
	private static boolean testOnBorrow =Boolean.parseBoolean(PropertiesUtil.getProperty("redis.test.borrow","true")); //在borrw一个jedis实例的时候，是否要进行验证操作，如果赋值true.则得到的jedis实例是可以用的
	private static boolean testOnReturn =Boolean.parseBoolean(PropertiesUtil.getProperty("redis.test.return","false")); //在return一个jedis实例的时候，是否要进行验证操作，如果赋值true.则放回的jedis实例是可以用的
	...
	private static void initPool() {
		...
		//如果有密码调用  info1.setPassword(密码);方法
	...
	public static void main(String[] args) {
		...
		System.out.println("progrom is end");
	}
